...
VRCManager::VRCManager(int geartimeout, STTDeliver *deliver, log4cpp::Category *logger, RT2DB* r2d)
	: m_sGearHost("127.0.0.1"), m_nGearPort(4730), m_GearTimeout(geartimeout), m_nSockGearman(0), m_deliver(deliver), m_Logger(logger), m_r2d(r2d)
{
...
VRCManager* VRCManager::instance(const std::string gearHostIp, const uint16_t gearHostPort, int geartimeout, STTDeliver *deliver, log4cpp::Category *logger, RT2DB* r2d)
{
	if (ms_instance) return ms_instance;

	ms_instance = new VRCManager(geartimeout, deliver, logger, r2d);
...
int16_t VRCManager::requestVRC(string& callid, uint8_t jobType, uint8_t noc = 1)
{
	...
	if (iter != vFnames.end()) {
		client = new VRClient(ms_instance, this->m_sGearHost, this->m_nGearPort, this->m_GearTimeout, *iter, callid, jobType, noc, m_deliver, m_Logger, m_r2d); // or VRClient(this);
